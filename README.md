...
`dsh-termux` is a set of install & maintenance scripts that let `dsh` run inside Termux. It installs the official Node.js **linux-arm64** binary on Termux's glibc runtime, so dsh itself never needs rebuilding: npm resolves the linux-arm64 prebuilt native modules directly, and the rare dependency that ships no prebuild (`fs-ext` in dsh 0.1.3+) is compiled in CI and delivered as a release asset that every install path overlays. A handful of small fixes adapt dsh to Android — the full list is the table below; the details live in [PATCHES.md](PATCHES.md).
...
**How far testing goes**: the author uses dsh-termux daily on one arm64 phone, where install, update, the patches and `dsh web` all work. CI applies the patches to the newest npm release and boot-smokes a fresh install on Linux whenever the patch inputs change and on the release-bound pull request that touches `VERSION` — but no automated step ever walks a real Termux install, and the prebuilt releases plus `install.sh` have not been tried on a second device.
...
- **CI**: every pull request runs `verify` — shell syntax, ShellCheck, patch-registry integrity, a patch-set applicability matrix across every dsh build the project serves (the pinned baseline, npm `latest`, npm `alpha`), the wrapper / `$BROWSER` generators, the `install.sh` delegation guard, the updater help contract, the test-harness entry point and the docs links — no npm install (the matrix only fetches a few registry tarballs), in well under a minute. The expensive half (`patch-check`: install dsh from npm, apply the patch set, boot smoke) runs only when it can change the answer: when the patch inputs change (`patches/`, `scripts/patch-lib.sh`, `NODE_VERSION`), on the release-bound pull request that touches `VERSION` — a stale patch set would get baked into the shipped artifact — and on demand via *Actions → patch-check → Run workflow* (where you can also point it at another npm spec). There is deliberately no schedule: the update paths stop loudly on a patch that no longer applies, so a nightly poll would only buy a run entry a day.
