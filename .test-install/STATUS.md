...
| 3b | 新入口 `run.sh` + 种子管理 | ✅ 冒烟 62（种子存储改内容寻址后 +20） |
...
| 脚本 | 通过项 | 覆盖一句话 |
|---|---|---|
| `tools/smoke-runner.sh` | 62 | 选择→前置→执行→补记→聚合→报告；五态归类；崩溃/空选择补 ERROR；人工项 fail-closed；**发布物输入实例解析失败 → UNMET 且不回退稳定版**；**候选产物前置接受归档或目录**；**种子：`set -u` 下的返回码、内容寻址、四分返回码、失败/中断不破坏已有种子、migrate 归位、staging 清扫（带反证）** |
