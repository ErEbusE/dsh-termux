...
> 那是 `DECISIONS.md`（**ADR-001..014**、实查更正 C1–C5、附录 A 的迁移映射）。
...
- **11 ①② / 11b / 11c / 11d 都已落地**（细节与"为什么"在 **ADR-001 落地记录**，此处只留结论）：
  `routes/`／`sandbox-lib.sh`／`baseline.env`／`release-test/` 全部删除（附录 A 的映射是唯一
  删除依据）；原生件机件全部下线；下限门禁在 `update-dsh.sh` 与 `02-install-dsh.sh` 两个入口
  生效（npm 改写树**之前**解析成一个精确版本）。
  **11d 的边界照 ADR-001 读**：`release-install/legacy-tarball`（真机 22 断言）证明的是
  "**当前安装器 × 这一个已发布旧 tarball**（`pre-dsh-0.1.3-alpha.2-g82a5fd6-1.2.8`，唯一带已
  编译原生件的历史版本）"的兼容性与自足可运行性，**不是**所有旧版本，也**不是**退役原生件机件
  的因果（被删代码不在这条执行路径上）。人工清单 `serve-legacy` 待人类实测。
- ✅ **第 10 项已落地**：`AGENTS.md` **71 行**（保留 §0–§6 编号——被 `tools/tb.sh`、
  `tools/pr-merge.sh`、`lib/sandbox.sh`、`verify.yml`、`PATCHES.md` 等**按名引用**）；
  `.test-install/README.md` **186 行**；都在 ADR-007 预算内，遵守**单点描述**。
  同批清掉的**过期事实**（都属"下一个人照它会得出错结论"）：两个 README 的"六条路线"、
  `CONTRIBUTING.md` 的"§5 命令拼写仍待重写"与**"未合并前无法 dispatch"**（已被 run
  35004183105 实测推翻）、`PATCHES.md` 的 R1/R2/R3 行号与对 `serve.sh` **当前**会 overlay 的
  陈述（ADR-010 之后它只启动冻结对象）、以及一个**不存在的 `--creds-env` 开关**。
  **§6.3 已与 ADR-007 对齐**（`644785c`）。
- ✅ **种子存储缺陷已修**（勿回退 #24、ADR-014）：内容寻址 ＋ 失败安全发布 ＋ 占名拒绝 ＋
  四分返回码。**两轮评审**：首轮双评审判 BLOCK（四条 blocker，`21f8d9f` 修掉）；修复后
  **复评**——安全/正确性组判 **BLOCK**（又抓到一个**数据损失**缺陷：`seed migrate` 在 CAS
  目标被损坏时删掉唯一与 pin 相符的旧副本），质量/架构组判 **CLEAR**（四条 blocker 确认关闭）。
  两个缺陷已在 **`81a455e`／`f8e1abc`** 全部修掉（含复评提出的 D2/D3/D4 与门禁分歧的另一半触发条件）。
  **修复本身仍是待人类实测。**
...
**细节在 `DECISIONS.md` 附录 A 与下面的 7c 落地记录表**。一句话：7a = 52 个断言组＋14 项公共
能力＋3 个探针逐条写明"谁继承了它"；7b = 三个探针迁入 `lib/probes.sh`，**触发 marker 按补丁
目标 rel 从被消费的注册表派生**（旧体系写死串的缺陷不再存在）；7c = executor ＋ 机制迁移
（L8/L9/L10）。当时"有登记无 executor"的缺口**已在 7f 补齐**。

### 当前执行顺序

**已完成**：11 ①②（`24a63bf`／`ab334a4`）→ 11b（`60c9f38`）→ 11c（`d8af293`）→ 11d（`69efdf5`）
→ 7f（`a2d3a64`）→ 第 9 项（`fc433d6`／`9680535`／`d6d070d`／`8439c9f`／`efca774`）
→ 第 10 项（文档重生成）→ 种子存储修复（`ae60c13`…`21f8d9f`，9 个提交）
→ **复评修复**（`81a455e`／`f8e1abc`，2 个提交）。
**各步的"为什么"在 ADR 里**（11b/11c → ADR-001；第 9 项 → ADR-006；种子存储 → ADR-014），
此处只留结果，不重复。

**第 9 项结果**（细节见 ADR-006）：`release.yml` 的打包三步提取成
`.github/scripts/package-runtime.sh`（`stage`/`verify`/`smoke`，两个 workflow 调用同一实现）；
新增 `.github/workflows/candidate-artifact.yml`（`contents: read`、**无任何输入能打开发布**）。
两条 candidate case 真机 PASS（16 ＋ 15 断言）。`tools/fetch-candidate.sh` 把 ADR-009 的
**证据绑定**做成一步命令，带假 gh 冒烟进 CI。

**第 10 项结果**：`AGENTS.md` → **71 行**（执行边界与证据协议，**保留被代码按名引用的 §0–§6
...
> - **首轮双 thermo-nuclear 评审（安全/正确性 ＋ 质量/架构，各自独立复现）都判 BLOCK**，四条
>   blocker 已全部修掉（`21f8d9f`）：**路径穿越**（越界 `.env` 条目让新增的 `seed migrate`
>   把库外文件 `mv`/`rm` 掉——**这是本次引入的新能力**，旧代码只读哈希）、`run.sh` 被推过
>   1000 行、严重度梯子写了三遍、`state.sh` 复制布局并**分类分歧**（损坏事实源在门禁判
>   UNMET、在加载器判 ERROR，与 ADR-003 矛盾）；
> - **修复本身又过了一轮复评**（`81a455e`／`f8e1abc`）：安全组对 `21f8d9f` 判 **BLOCK**——
>   另有一个**数据损失**缺陷（`seed migrate` 在 CAS 目标损坏时删掉唯一与 pin 相符的旧副本），
>   质量组判 **CLEAR**（四条 blocker 确认关闭）。复评另提的门禁分歧另一半、**空转的穿越断言**
>   （夹具少一层 `..`）、信号场景的 CI 预算、以及四个小 nit 全部修掉，冒烟 69 → **79**。
>   详细清单与四条教训在**勿回退 #24**；
> - 归类变化**不是放行**：`UNMET → 退出码 3 → INCOMPLETE`，只有 PASS 给 READY；
> - **人工那一轮要加种子路径的项**：pin A → pin B（不得覆盖 A 的字节）→ 同名重钉被拒
>   （`--force` 才放行）→ 发布中途杀掉再重跑（旧种子仍可用、无残留 staging）→ 旧 pin 记录
>   仍能被 `seed_load` 消费 → **越界条目不得动到库外文件**。
>   **在人类看过它之前：不冻结产物对象、不写 `Tested-by`、不合并。**
...
| 脚本 | 通过项 | 覆盖一句话 |
|---|---|---|
| `tools/smoke-runner.sh` | 79 | 选择→前置→执行→补记→聚合→报告；五态归类；崩溃/空选择补 ERROR；人工项 fail-closed；**发布物输入实例解析失败 → UNMET 且不回退稳定版**；**候选产物前置接受归档或目录**；**种子：`set -u` 下的返回码、内容寻址、四分返回码、失败/中断不破坏已有种子、migrate 归位、**坏 CAS 对象占位时 migrate 不得删掉唯一可用的旧副本**、零条目/缺 `SEED_DSH_VERSION` 的事实源在**三处判据一致**、staging 清扫、**信号 trap 必须清理并终止**（带反证）** |
...
24. **种子资产曾是"扁平 + 同名"存储，`seed set` 会静默毁掉已有种子的字节**（2026-09-15 实测复现；
    ...
    **修法（顾问 `gpt-6-astra` 裁决 ＋ 用户拍板；完整决定见 ADR-014）**：① 资产改**内容寻址** `seed-assets/<sha256>/<名>`
    ...
    把产品里的 `exit 143` 删掉该断言必须变红）。
    迁移：`run.sh seed migrate` 把与 pin 逐字相符的旧扁平资产归位（真实资产已归位，`stable` 仍 ASSET-OK）。
    **首轮双评审（安全 ＋ 质量，各自独立复现）都判 BLOCK，四条 blocker 已在 `21f8d9f` 修掉**：
    **路径穿越**（越界 `.env` 条目让 `seed migrate` 移动/删除**库外**文件——本次引入的新能力；修法
    是在**构造函数**强制形状（唯一解析器 ＋ `seed_cas_path` 各自拒一次），不是靠注释）、`run.sh`
    被推过 1000 行（发布流程提取成 `seed_publish`，回到 987）、严重度梯子三处重复（收敛为
    `seed_worst`）、`state.sh` 复制布局且**分类分歧**（委托 `seed_present` 后两处一致）。

    **复评（针对修复本身）又抓出四类问题，已在 `81a455e`／`f8e1abc` 修掉**——审查修复的改动
    与审查原始改动同样必要，这一轮每条都是"修的时候新引入或没修全"：
    - **数据损失（安全组判 BLOCK 的那条）**：`seed_migrate_legacy` 在 CAS 目标**已存在**时
      只判存在性就 `rm -f "$src"`，从不比内容。目录名是哈希、内容被损坏（截断/坏盘/手工改动）时，
      唯一还与 pin 相符的字节就是那份旧扁平文件——**被"修复命令"删掉还打印"归位"记成功**，
      状态从可修复(FAIL=1)退化成无从恢复(UNMET=3)。修法与 `seed_install_cas` 同判据。
      教训：**"目标已存在"不等于"目标正确"**；凡是删除动作，删除依据必须是内容比对，不是存在性。
    - **门禁分歧的另一半**：上一轮只对齐了"记录形状非法"，**零条目**与**缺 `SEED_DSH_VERSION`**
      两个触发条件仍让 `seed list`/`seed show` 报"资产齐、哈希相符"，而消费它的 case 一律 ERROR。
      三处（`seed_present`/`seed_verify`/`seed_load`）现在判据完全一致。
      教训：**修分歧要按"所有触发条件"修，不是按"报告里那一个例子"修**。
    - **回归断言可以是空转的**：穿越场景的夹具写成 `../../`，而受害文件在 `$SCRATCH/cas-victim`，
      从 `$tdir/seeds/seed-assets` 要**三层**才回到 `$SCRATCH`——所以记录指向一个不存在的路径，
      迁移什么都不做，**"不得移动库外文件"这条断言对未修复的库照样通过**。改成 `../../../`
      后，未修复的库真的会把库外文件移走（反证已实测）。
      教训：**"断言没红"要先证明它在该红时会红**；夹具的相对深度必须实测对齐。
    - **信号场景吃掉 CI 预算**：`bash` 要等前台子进程结束才处理 trap，所以 stub 的 `sleep 30`
      就是"终止被推迟"的时长的度量——套件从 17s 涨到 42s，逼近 `static` 的 1 分钟预算。
      改 `sleep 5` 后套件 19s（七个冒烟共 38s），被测性质不变。
      教训：**测"行为"不需要付"时长"的代价**；夹具里的大常数要问一句"它到底在测什么"。
    另外顺手关掉：CAS 目标存在但**不是普通文件**时 `mv` 会静默把源塞进目录却照常发布记录
    （改为拒绝）、`seed_load` 的循环字段重新声明为 `local`（该文件自己写着"调用者作用域残留
    是最危险的一种"）、一个未使用的 `dl` 绑定、`dead_pid` 未 `local`。

    教训推广：① **"两个名字指向同一份字节"这种记录层承诺，必须在字节层也成立才算兑现**；
    ② **"路径不是信任依据"必须包括"构造路径的那段文本"**——只校验内容不校验形状是不够的；
    ③ **修复本身要再评审一轮**：这四条全是"改这个缺陷时新引入或没改全"的，只审原始改动会漏掉。

### 证据边界 / 交接必知

> 这里**不是待办清单**（待办只剩进度表的第 12 项）——是**"报告能主张什么"的边界**，
> 换人后最容易读强的地方。
