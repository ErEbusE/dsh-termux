...
- ✅ **种子存储缺陷已修**（勿回退 #24、ADR-014）：内容寻址 ＋ 失败安全发布 ＋ 占名拒绝 ＋
  四分返回码；**双评审都判 BLOCK，四条 blocker 已修**（`21f8d9f`，含一个本次引入的**路径穿越**
  安全缺陷）。**修复本身仍是待人类实测。**
...
| 3b | 新入口 `run.sh` + 种子管理 | ✅ 冒烟 69（种子存储改内容寻址 + 双评审修复后 +27） |
...
> - **双 thermo-nuclear 评审（安全/正确性 ＋ 质量/架构，各自独立复现）都判 BLOCK**，四条
...
| 脚本 | 通过项 | 覆盖一句话 |
|---|---|---|
| `tools/smoke-runner.sh` | 69 | 选择→前置→执行→补记→聚合→报告；五态归类；崩溃/空选择补 ERROR；人工项 fail-closed；**发布物输入实例解析失败 → UNMET 且不回退稳定版**；**候选产物前置接受归档或目录**；**种子：`set -u` 下的返回码、内容寻址、四分返回码、失败/中断不破坏已有种子、migrate 归位、staging 清扫、**信号 trap 必须清理并终止**（带反证）** |
...
24. **种子资产曾是"扁平 + 同名"存储，`seed set` 会静默毁掉已有种子的字节**（2026-09-15 实测复现；
    ...
    **双评审（安全 ＋ 质量，各自独立复现）都判 BLOCK，四条 blocker 已在 `21f8d9f` 修掉**：
