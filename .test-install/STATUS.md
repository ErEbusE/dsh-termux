...
24. **种子资产是"扁平 + 同名"存储，`seed set` 会静默毁掉已有种子的字节**（2026-09-15 实测复现，
    **未修**）：`seed_fetch_assets` 按**固定资产名**写进**同一个** `seeds/seed-assets/`，与种子名无关。
    两个已复现的失效（用假字节在隔离沙箱复现，真实资产未受影响）：
    - **跨种子**：pin `stable`=tagA（`seed_verify` rc=0）→ pin `other`=tagB → `stable` 变成
      `ASSET-CHANGED`／rc=1。两个 `.env` 都在，**外观上"并存"，字节上已被覆盖** —— ADR-004
      "旧种子保留"在**记录层**成立、在**字节层**被违反。
    - **单种子**（**这条不依赖多种子**）：`seed_fetch_assets` 的 `.part`→`mv` 是**逐资产**原子的，
      不是**每颗种子**原子的。re-pin 时资产 1 已 `mv`、资产 2 下载失败 → `seed_write_env` 不执行，
      `.env` 仍是**旧 pin**，而旧种子的字节已变 → 之前全绿的种子 rc=1，且**两个 tag 都没有有效 pin**。
    可达性：只有人手敲 `run.sh seed set` 会触发（`verify`/`check` 从不调用它），当前只有 1 颗种子，
    因此对第 12 项交付是**潜伏**风险、不阻塞它。**但"只有一颗种子"不能消除第二个失效。**
    已知程度：`cases/release-install-legacy-tarball.sh` 的头部**已经**把"扁平目录会被覆盖"写成它
    必须当场下载、不借种子路径的理由 —— 即**已知并绕过**，却**从未进任何台账**（STATUS/DECISIONS/
    本清单都没有）。本条就是补这个记录；**修复方案未定**（顾问 `gpt-6-astra` 2026-09-15 裁决：
    资产改**内容寻址**存储、**失败的 pin 绝不许破坏已有种子**、`.env` 最后发布；并把"占用名不得换
    pin"与文档**同步**改，否则就等于静默废掉 ADR-004；他同时承认"有护栏的延后"也可辩护）。
