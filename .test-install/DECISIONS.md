...
2. **"旧版本走自己的 tarball"这条路径零覆盖 → 仍未做（11d）**：`build/install.sh` 会从
   **所选 tarball** 里加载它自己的 `common.sh`（调用关系上不受本次删除影响），但那只是
   静态核对，不是实测。需要一条"**当前安装器 × 对应旧 tarball**"的隔离回归，且**不得**
   通过 overlay 当前脚本把被测旧产物偷偷换掉。排在**第 9 项之后、第 12 项之前**；在它
   落地前，**不得**把"旧版本仍可安装"当作已验证的结论写进交付证据。
...
> **仍未做**的是 11b 带回的第二个缺口（旧 tarball 安装回归——`STATUS.md` 进度表的
> **11d**；第一个，更新目标下限检查，已在 11c 落地）。本清单保留原文，作为"每条缺口当时是怎么被识别出来的"的记录；
> **执行时以上面的状态为准**。
