...
## ADR-015 交付模型收窄：删除冻结对象/轮次/签认，serve 退化为纯启动器

**背景**：ADR-010 引入的「冻结对象 ＋ 轮次 ＋ 观察台账 ＋ `finalize --observed`」在
本地单人适配流程里被维护者判定为**过度验证**（2026-09-16，PR #38 维护者评审）。

**否决的理由（实测支撑）**：

1. **它挡不住它声称要挡的东西**。ADR-010 自己写着「人是不是真的测了仍是**流程信任**
   ——agent 有 shell 就能启动 serve」。既然挡不住伪造，它买到的是**文书**（台账/轮次/
   对象），而文书本身又制造了大量约束。
2. **`source_digest` 是实现层加码，不是决策要求**。回看 ADR-010 的「身份三层」：
   `build_digest`（哪组输入）、载荷内容摘要（哪些字节）、`run_id`/`case_id`（哪条过程）
   ——**里面没有「整棵工作树的源码摘要」**。而它连 `*.md` 都算：改一句文档就「源漂移」
   拒绝启动，挡不住任何真实失效（无运行时脚本读、无发布物包含仓库 `.md`），只制造
   「先冻结就不能再动文档」的伪约束。
3. **`serve.sh` 职责错位**。它应当是**给人类用的、隔离环境下的沙箱启动器**：设置隔离
   环境变量、决定是否复制凭据、启动 `dsh web`。沙箱内运行时的准备（打上补丁的那一版）
   **是 agent 的职责**。而旧版 581 行里只有约三分之一在真正启动，其余是 manifest 解析、
   载荷校验、观察台账、漂移闸门、轮次归属、finalize 提示。
4. **外部字节才是真正需要核验的**：下载的发布物（种子逐件 sha256、`download-path` 的
   「下载字节 == pin」）与 npm 的 SRI 闭环。本地源码不冻结。

**决定**：

- **删除** `lib/frozen.sh`、`tools/smoke-frozen.sh`、`run.sh finalize`、轮次记录
  （`write_round`/`round_get`/`round_dir`）、观察台账、`DSH_HUMAN_*` 间接层、`--freeze`、
  `source_digest`（含 `frozen_source_ok` 与其在 serve 的硬闸门）、旧开关 `legacy_guard`。
- **`serve.sh` 退化为纯启动器**（约 154 行）：`--sandbox <名> [--port <n>] [--with-creds]
  [--no-open]`。它解析沙箱、校验载荷入口在场、`mkdir` 可写区、用 `sandbox_env_human`
  隔离、写启动器（写在载荷之外）、起 `dsh web`；**保留**线上 runtime 起止守卫
  （ADR-008 的检测层——那保护的是维护者自己的安装，与人工签认无关）。
- **`state_verdict` 只判自动层**：`REJECTED`（有 FAIL/ERROR）/`INCOMPLETE`（有 UNMET）/
  `READY`（无）。人类实测**不进**自动层结论。
- **交付留痕**：人类在沙箱实测后，按 AGENTS.md §6 用 `tools/tb.sh` 把 `Tested-by:`
  写进**合并提交**。纯 CI/工作流改动用 `tb.sh --review`；**能落到设备上的改动一律**
  `on-device`。
- **`human` 列保留在 registry**，但语义降为「这条 case 该由人实测什么」的**文档**
  （`cases/checklists/<id>.txt` 索引），**不再是任何自动门的输入**；带人工清单的 case
  通过后保留沙箱并把 `serve.sh --sandbox <名>` 命令打印出来。

**流程（维护者需求，照此执行）**：现象 → agent 分析根源 → 建修改分支 → 改代码 + 语法检查
→ dry-run 沙箱（原版 npm 包 + 修改）启动测试 → 修到正常 → 交人类用沙箱实测 → 反复 →
本轮适配结束 → 工作区补丁 → 打包补丁集做**升级测试** → 交人类确认 → 推远程开 PR →
CI 打包**验证产物**（结构同 release）→ 用 `install.sh` 在沙箱做**全新安装** → agent 查报错
→ 交人类实测 → 合并 PR。有问题：人反馈 → 判定打包问题还是补丁问题 → 重走。

**保留（不是过度验证）**：18 条 case executor、外部字节核验（种子/SRI/下载 sha）、
线上 runtime 守卫、启动器生成器（`scripts/common.sh` 唯一实现）、其余六个 smoke。

**边界**：删掉的是**自动层的人工签认链**，不是人类实测本身——人类实测仍在真机上做，
证据仍留在会话与 `Tested-by` 里。自动层的 `verify` 仍按 diff 派生必需 case。

**落地**：本 ADR 随 PR #38 的修改提交落地；ADR-010 的「身份三层」正文**保留**（载荷摘要
与输入身份仍然成立），但其「轮次/观察台账/finalize」部分已被本 ADR 取代。
